...
An attempt to solve this problem has been made by \todo{cite RT-ROS chinese}, but we've deemed it as an unnecessarily complex workaround and decided to explore different solutions.
...
The benefits of \texttt{PREEMPT\_RT}  are invaluable: at the cost of throughput, unbounded latencies are virtually eliminated and the performance benefits are notable\cite{dmoceri-benchmarking-rtlinux}.\todo{Expand upon the topic}
...
The CBS algorithm is used in conjunction with EDF for...\todo{Dig up details about CBS}

%% how does sched-edf work
The algorithms behind SCHED\_EDF are described by.\todo{citetext  *<abeni, buttazzo> }
...
%\includegraphics[]{img/overview.svg} %svg->png
...
Logging was implemented with syslog() and it's got better performance than printf() logging, besides it can make use of system logs and be easily trackable with tools like journalctl. Refer to *<article that says that printf is worse than syslog>\todo{cite}
...
\newpage
% Ch3
\chapter{Implementation Issues}\todo{uhhh what}
...
\todo{Describe otto266 better OR delete this section entirely, incorporating it somewhere else}
...
\todo{I'm not sure I'm keeping this section as it's pretty hard to find reputable sources, but it's a nice thing to do anyway.}
...
\todo{We have not explored cpusets after all. Remove?}
